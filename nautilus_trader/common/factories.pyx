...
cdef class OrderFactory:
    ...
    cpdef MarketOrder market(
        ...
        fill_price_override : Price, optional
            Deterministic fill-price override (spec 145). When set, the
            backtest matching engine fills this MARKET order at exactly this
            price (bypassing OHLC tick simulation). The strategy is contract-
            responsible for validation — NT does not range-check. Field is
            excluded from serialization to prevent external replay injection.
